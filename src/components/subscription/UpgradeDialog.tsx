                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        <Icon className={`h-4 w-4 ${
                          selectedChannels.includes(channel.id) ? 'text-white' : 'text-gray-600'
                        }`} />
                        <span className="text-sm font-medium">{channel.name}</span>
                      </div>
                      {selectedChannels.includes(channel.id) && channel.isAvailable && (
                        <Check className="h-4 w-4" />
                      )}
                    </div>
                    <div className="flex items-center gap-2">
                      <div className="w-4" /> {/* Spacer to align with icon */}
                      <span className={`text-xs ${
                        selectedChannels.includes(channel.id) ? 'text-blue-100' : 'text-gray-500'
                      }`}>
